...
(command-line
 ...
 (define-values (times min-distance)
   ...
   ;; since distance is symmetric. This does mean we do fewer distance
   ;; calculations than the C program (1953/3906 on input.txt).
   (for/fold ([times 0] [min-distance #f])
             ([xy (in-combinations args 2)])
     (match-define (list x y) xy)
     (define dist (levenshtein-distance x y))
     (values (fx+ 1 times)
             (if min-distance (fxmin min-distance dist) dist))))
